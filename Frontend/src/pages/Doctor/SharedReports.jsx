...
const DoctorSharedReports = () => {
  ...
  const shareStats = useMemo(() => {
    const now = Date.now();
    const activeShares = (sharedReports || []).filter((share) => {
      if (!share?.expiresAt) {
        return true;
      }
    ...
    const totalReports = (sharedReports || []).reduce(
      (sum, share) => sum + (share?.reportIds?.length || 0),
      0
    );

    const totalNotes = (sharedReports || []).reduce(
      (sum, share) => sum + (share?.notes?.length || 0),
      0
    );

    return {
      totalShares: (sharedReports || []).length,
  ...
  const availableCategories = useMemo(() => {
    ...
    (sharedReports || []).forEach((share) => {
      (share?.reportIds || []).forEach((report) => {
        if (typeof report === "object" && report?.category) {
  ...
  const filteredShares = useMemo(() => {
    ...
    return (sharedReports || []).filter((share) => {
      const patientName = (share?.patientName || "").toLowerCase();
      const reportNames = (share?.reportIds || [])
        .map((report) =>
          typeof report === "object" ? report.originalFilename || "" : "Report"
        )
        .join(" ")
        .toLowerCase();

      const searchMatch = !q || patientName.includes(q) || reportNames.includes(q);

      const isActive = !share?.expiresAt || new Date(share.expiresAt).getTime() > now;
      const statusMatch =
        statusFilter === "all" ||
      ...
      const categoryMatch =
        categoryFilter === "all" ||
        (share?.reportIds || []).some(
          (report) => typeof report === "object" && report?.category === categoryFilter
        );

      return searchMatch && statusMatch && categoryMatch;
    });
  }, [sharedReports, searchTerm, statusFilter, categoryFilter]);
  ...
  const handleAddNote = async () => {
    if (!noteForm.text.trim() || !noteForm.shareId) {
      return;
    }
  ...
  return (
    <DoctorDashboardLayout pageTitle="Shared Reports">
      <div className="space-y-6" style={{ fontFamily: "'Inter', sans-serif" }}>
        ...
        <section
          ...
          style={{
            background:
              "linear-gradient(135deg, #0f172a 0%, #1e1b4b 45%, #252A61 100%)",
        ...
        >
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5">
            <div className="flex items-center gap-3">
              ...
              >
                <FiShield style={{ color: "#fff", fontSize: "1.35rem" }} />
              </div>
              <div>
                <h1
                  style={{
                ...
                >
                  Review patient-shared records, add structured notes, and track expiry.
                </p>
              </div>
            </div>
            ...
            >
              <FiRefreshCw className={sharedReportsLoading ? "animate-spin" : ""} />
              Refresh
            </button>
          </div>
          ...
          <div className="mt-5 grid grid-cols-2 xl:grid-cols-4 gap-3">
            {[
              {
                label: "Total Shares",
                value: shareStats.totalShares,
                icon: FiShield,
                tint: "#a5b4fc",
              },
              {
                label: "Active Access",
                value: shareStats.activeShares,
                icon: FiActivity,
                tint: "#6ee7b7",
              },
              {
                label: "Shared Files",
                value: shareStats.totalReports,
                icon: FiFileText,
                tint: "#93c5fd",
              },
              {
                label: "Notes Added",
                value: shareStats.totalNotes,
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <item.icon style={{ color: item.tint, fontSize: "0.95rem" }} />
                </div>
                <p
                  style={{
                    marginTop: 6,
        ...
        <section className="grid grid-cols-1 2xl:grid-cols-[minmax(0,1fr)_320px] gap-6">
          <div className="space-y-4">
              ...
              <div className="flex flex-col lg:flex-row gap-3 lg:items-center">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    placeholder="Search by patient name or report file"
                ...
                <div className="flex gap-2 flex-wrap">
                  ...
                  >
                    <FiSliders style={{ color: "#64748b", fontSize: "0.82rem" }} />
                    <select
                      value={statusFilter}
                  ...
                  <div
                    ...
                    style={{ background: "#f8fafc", border: "1px solid #e2e8f0" }}
                  >
                    <FiFilter style={{ color: "#64748b", fontSize: "0.82rem" }} />
                    <select
            ...
            {!sharedReportsLoading && filteredShares.length === 0 && (
              ...
              >
                ...
                >
                  No matching shared reports
                </p>
                <p style={{ fontSize: "0.82rem", color: "#94a3b8" }}>
                  Try changing filters or wait for patients to share new records.
                </p>
              </div>
            )}
                                    ...
                                    style={{
                                      background:
                                        noteForm.type === nt.value
                                          ...
                                          : "#f1f5f9",
                                      color:
                                        noteForm.type === nt.value ? "#6366f1" : "#64748b",
                                      border:
                                        noteForm.type === nt.value
          ...
          <aside
            className="rounded-2xl p-4 h-fit 2xl:sticky 2xl:top-24"
            style={{
              background: "#ffffff",
              border: "1px solid #e6ebf3",
              boxShadow: "0 5px 20px rgba(15,23,42,0.05)",
          ...
          >
            ...
            >
              At-a-glance context while reviewing shared records.
            </p>
                ...
                >
                  ...
                  >
                    {(sharedReports || []).reduce(
                      (sum, share) =>
                        sum +
                        (share?.notes || []).filter((note) => note.type === type.value)
                          .length,
                      0
                    )}
                  </span>
            ...
            >
              ...
              <ol
                ...
                style={{ fontSize: "0.76rem", color: "#334155", fontWeight: 500 }}
              >
                <li>1. Expand a patient share card.</li>
                <li>2. Open each report and inspect evidence.</li>
                <li>3. Add typed notes for action tracking.</li>
                <li>4. Re-check before share expiry window ends.</li>
              </ol>
            </div>
          </aside>
        </section>
            ...
            >
              ...
              >
                <div>
                  ...
                  >
                    Shared by {reportViewer.share?.patientName} • {reportViewer.report.category}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  >
                    <FiExternalLink style={{ color: "#fff", fontSize: "0.9rem" }} />
                  </a>
                  <button
                    onClick={() =>
              ...
              <div className="flex-1 overflow-auto" style={{ minHeight: 0, background: "#f8fafc" }}>
