...
import AlertsSection from './sidebarSections/AlertsSection'
import OverviewSection from './sidebarSections/OverviewSection'
import PharmacySection from './sidebarSections/PharmacySection'
import PrescriptionsSection from './sidebarSections/PrescriptionsSection'
import ReportsSection from './sidebarSections/ReportsSection'
import SettingsSection from './sidebarSections/SettingsSection'
import TrustSection from './sidebarSections/TrustSection'
import VerificationSection from './sidebarSections/VerificationSection'
...
const PATIENT_SIDEBAR_ROUTE_SECTIONS = new Set([
  'overview',
  'reports',
  'trust',
  'prescriptions',
...
const PatientDashboardPage = () => {
  ...
  const [contentNav, setContentNav] = useState('overview')
  ...
  const sectionFromRoute = (section ?? '').toLowerCase().trim()

  const navigateToSection = (navId, replace = false) => {
    const safeNav = PATIENT_SIDEBAR_ROUTE_SECTIONS.has(navId) ? navId : 'overview'
    const targetPath = safeNav === 'overview' ? '/patient-dashboard' : `/patient-dashboard/${safeNav}`
  ...
  useEffect(() => {
    if (!sectionFromRoute) {
      ...
      setContentNav('overview')
      return
    }

    if (!PATIENT_SIDEBAR_ROUTE_SECTIONS.has(sectionFromRoute)) {
      navigate('/patient-dashboard', { replace: true })
      return
    }

    setActiveNav(sectionFromRoute)
    setContentNav(sectionFromRoute)

    if (sectionFromRoute === 'reports' || sectionFromRoute === 'prescriptions' || sectionFromRoute === 'pharmacy') {
      setOverviewSection(sectionFromRoute)
    }
  }, [sectionFromRoute, navigate])
  ...
  const openAlertsMenu = () => {
    setActiveNav('alerts')
    setContentNav('alerts')
    navigateToSection('alerts')
  }

  const openReportVault = () => {
    setOverviewSection('reports')
    setActiveNav('reports')
    setContentNav('overview')
  }

  const openPrescriptions = () => {
    setOverviewSection('prescriptions')
    setActiveNav('prescriptions')
    setContentNav('overview')
  }

  const openPharmacy = () => {
    setOverviewSection('pharmacy')
  ...
  const handleSidebarNavigation = (navId) => {
    ...
    if (navId === 'reports' || navId === 'prescriptions' || navId === 'pharmacy') {
      setOverviewSection(navId)
    }
  ...
  const renderMainContent = () => {
    switch (contentNav) {
      case 'reports':
        return (
          <ReportsSection
            ...
            onAnalyzeReport={handleAIAnalyzer}
            onToggleSaveReport={handleToggleSaveReport}
            onDeleteReport={handleDeleteReport}
            aiInsight={aiInsight}
            onCloseAiInsight={closeAiInsight}
          />
        )
      case 'prescriptions':
        return (
          <PrescriptionsSection
            ...
            onUpdatePharmacyStatus={updatePharmacyStatus}
          />
        )
      case 'pharmacy':
        return (
      ...
      case 'alerts':
        return <AlertsSection notifications={notifications} focused />
      case 'verification':
        return <VerificationSection prescriptions={prescriptions} />
      case 'settings':
        return <SettingsSection patientProfile={patientProfile} onOpenProfileEditor={openProfileEditor} />
      case 'trust':
        return <TrustSection trustScore={trustScore} prescriptions={prescriptions} />
      case 'overview':
      default:
        return (
          <OverviewSection
            overviewSection={overviewSection}
            onOpenReportVault={openReportVault}
    ...
    <div className={`pt-page ${themeMode === 'dark' ? 'is-dark' : ''}`}>
      ...
      <div className='pt-shell'>
        <aside className='pt-sidebar'>
          ...
          <div className='pt-sidebar-tail' style={{ marginTop: 'auto', display: 'grid', gap: '0.55rem' }}>
            <div className='pt-sidebar-footer' style={{ marginTop: 0 }}>
              <p>Identity Integrity</p>
              <strong>Verified Session</strong>
            </div>

            <button type='button' className='pt-nav-item' onClick={handleLogout}>
              <span className='pt-nav-icon' aria-hidden='true'>
                <LuLogOut />
              </span>
              <span>Logout</span>
