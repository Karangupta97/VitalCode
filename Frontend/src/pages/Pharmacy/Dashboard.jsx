...
const statCards = [
  {
    label: 'Medicines Sold Today',
    value: '148',
    sub: 'across 34 prescriptions',
  ...
  },
  {
    label: 'Stock Alerts',
    value: '3',
    sub: 'Metformin, Insulin, Vit B12',
    trend: 'Needs action now',
    trendColor: '#A32D2D',
  },
];
...
const CustomChartTooltip = ({ active, payload, label }) => {
  ...
  return (
    ...
    >
      <p style={{ fontSize: '0.78rem', fontWeight: 700 }}>{`${dayName} — ${row.units} units sold — ₹${row.revenue.toLocaleString('en-IN')}`}</p>
    </div>
  );
};
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        >
          ...
          <div className="relative z-10 p-5 sm:p-7 lg:p-9">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full" style={{ background: 'rgba(255,255,255,0.14)', border: '1px solid rgba(255,255,255,0.22)' }}>
              ...
              <span style={{ color: '#fff', fontSize: '0.74rem', fontWeight: 700 }}>Pharmacy Command Center</span>
            </div>
            ...
            <div className="flex flex-wrap gap-2.5 mt-5">
              {[
                { label: 'Pending Scans', value: 5 },
                { label: 'Low Stock Alerts', value: 3 },
                { label: 'Doctor Requests', value: 7 },
              ].map((item) => (
                <div
                  key={item.label}
                ...
                >
                  <span style={{ color: '#e0f2fe', fontSize: '0.74rem', fontWeight: 600 }}>{item.label}</span>
        ...
        >
          {statCards.map((card) => (
            ...
            >
              <p style={{ color: '#64748b', fontSize: '0.76rem', fontWeight: 600, letterSpacing: '0.02em' }}>{card.label}</p>
              <p style={{ color: '#0f172a', fontSize: '1.85rem', fontWeight: 800, lineHeight: 1.1, marginTop: 7 }}>{card.value}</p>
              <p style={{ color: '#94a3b8', fontSize: '0.76rem', fontWeight: 500, marginTop: 6 }}>{card.sub}</p>
              {card.trend && (
                <div className="inline-flex items-center gap-1 mt-3">
                  <ArrowUpRight style={{ width: 14, height: 14, color: card.trendColor }} />
                  <span style={{ color: card.trendColor, fontSize: '0.75rem', fontWeight: 700 }}>{card.trend}</span>
                </div>
              )}
            </div>
          ))}
        ...
        <motion.section
          ...
          style={dashboardCardStyle}
        >
          <h3 style={{ color: '#0f172a', fontSize: '1rem', fontWeight: 800 }}>Medicines Sold - This Week</h3>
          <div style={{ height: 310, marginTop: 14 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={weeklySales} margin={{ top: 6, right: 10, left: -10, bottom: 0 }}>
                ...
                <Tooltip content={<CustomChartTooltip />} cursor={{ fill: 'rgba(15,118,110,0.08)' }} />
          ...
          <div className="flex flex-wrap gap-2.5 mt-3">
            {[
              'Top Medicine: Paracetamol 650mg',
              'Peak Day: Wednesday',
              'Avg Daily: 162 units',
            ].map((pill) => (
              <span
        ...
        <section className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          ...
          >
            <div className="px-4 py-3.5" style={{ borderBottom: '0.5px solid rgba(0,0,0,0.08)', background: '#f8fbff' }}>
              <h3 style={{ color: '#0f172a', fontSize: '0.9rem', fontWeight: 800 }}>Recent Dispensed Prescriptions</h3>
            </div>
              ...
              <table className="w-full">
                <thead>
                  <tr style={{ borderBottom: '0.5px solid rgba(0,0,0,0.08)' }}>
                    {['Patient', 'Doctor', 'Medicine', 'Qty', 'Time', 'Status'].map((head) => (
                      <th key={head} className="text-left px-4 py-2.5" style={{ fontSize: '0.68rem', fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                        {head}
                      </th>
                    ))}
                  </tr>
                  ...
                  {recentDispensed.map((row) => (
                    <tr key={`${row.patient}-${row.time}`} style={{ borderBottom: '1px solid #f1f5f9' }}>
                      ...
                      <td className="px-4 py-2.5">
                        <span
                          data-status={row.status.toLowerCase().replace(/\s+/g, '-')}
                          className={`inline-flex px-2.5 py-1 rounded-full status-pill vc-status-badge status-${row.status.toLowerCase().replace(/\s+/g, '-')}`}
                          style={{
                            fontSize: '0.69rem',
                            fontWeight: 700,
          ...
          >
            <div className="px-4 py-3.5" style={{ borderBottom: '0.5px solid rgba(0,0,0,0.08)', background: '#f8fbff' }}>
              <h3 style={{ color: '#0f172a', fontSize: '0.9rem', fontWeight: 800 }}>Low Stock Alerts</h3>
            </div>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p style={{ color: '#64748b', fontSize: '0.74rem', marginTop: 3 }}>
                          Current Stock: <span style={{ fontWeight: 700 }}>{item.currentStock}</span> | Reorder Level:{' '}
        ...
        <section className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          ...
          >
            <div className="px-4 py-3.5" style={{ borderBottom: '0.5px solid rgba(0,0,0,0.08)', background: '#f8fbff' }}>
              <h3 style={{ color: '#0f172a', fontSize: '0.9rem', fontWeight: 800 }}>Pending Doctor Stock Requests</h3>
            </div>
              ...
              {pendingRequests.map((req) => (
                <div key={`${req.doctor}-${req.requested}`} className="rounded-xl px-3 py-3" style={{ border: '1px solid #e8edf5', background: '#fff' }}>
                  <div className="grid grid-cols-1 sm:grid-cols-5 gap-3 items-center">
                    ...
                    <p style={{ color: '#0f172a', fontSize: '0.76rem', fontWeight: 700 }}>Qty: {req.qty}</p>
            ...
            <div className="px-4 pb-4">
              <button style={{ color: RX_COLORS.primary, fontSize: '0.78rem', fontWeight: 700 }}>View All Requests</button>
            </div>
          </motion.div>
          ...
          >
            <div className="px-4 py-3.5" style={{ borderBottom: '0.5px solid rgba(0,0,0,0.08)', background: '#f8fbff' }}>
              <h3 style={{ color: '#0f172a', fontSize: '0.9rem', fontWeight: 800 }}>License Status</h3>
            </div>
            ...
            <div className="p-6 text-center">
              {pharmacy?.licenseStatus === 'expired' || pharmacy?.licenseStatus === 'pending' ? (
                <>
                  ...
                  <p style={{ color: '#dc2626', fontWeight: 800, marginTop: 10 }}>License {pharmacy?.licenseStatus === 'expired' ? 'Expired' : 'Pending Review'}</p>
                  <p style={{ color: '#64748b', fontSize: '0.82rem', marginTop: 8 }}>
                    License No: {pharmacy?.licenseNumber || 'PH-MH-20345'}
                  </p>
                  <button
                ...
                <>
                  ...
                  <p style={{ color: '#004A99', fontWeight: 800, marginTop: 10, fontSize: '1.06rem' }}>License Verified</p>
                  <div className="space-y-1.5 mt-3">
                    <p style={{ color: '#334155', fontSize: '0.8rem' }}>License No: <span style={{ fontWeight: 700 }}>{pharmacy?.licenseNumber || 'PH-MH-20345'}</span></p>
                    <p style={{ color: '#334155', fontSize: '0.8rem' }}>Valid Until: <span style={{ fontWeight: 700 }}>31 Dec 2026</span></p>
                    <p style={{ color: '#334155', fontSize: '0.8rem' }}>Issuing Body: <span style={{ fontWeight: 700 }}>Maharashtra State Pharmacy Council</span></p>
                  </div>
                  ...
                  <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full mt-3" style={{ background: '#E6F4FF', border: '1px solid rgba(0,74,153,0.25)' }}>
                    ...
                    <span style={{ color: '#004A99', fontSize: '0.73rem', fontWeight: 700 }}>Active</span>
                  </div>
                </>
              )}
            </div>
